...
export function OwnerTable({ data }: OwnerTableProps) {
  ...
  const handleToggleStatus = async (admin: Admin) => {
    try {
      ...
      await dispatch(
        updateAdminStatus({ id: admin.id, is_active: !admin.is_active }),
      ).unwrap();
    } catch (error) {
                ...
                {owner.businesses_count > 0 && (
                  <div className="mt-1 flex flex-col gap-1">
                    {owner.businesses.slice(0, 2).map((b) => (
                      <span
                        key={b.id}
                        className="text-xs text-brand-muted truncate max-w-[150px]"
                      >
                        • {b.name}
                      </span>
                    ))}
                  ...
                  <Button
                    ...
                    className={
                      owner.is_active
                        ...
                        : "text-brand-success border-brand-success/20 hover:bg-brand-success/10"
                    }
                  >
                    {owner.is_active ? (
                      <>
