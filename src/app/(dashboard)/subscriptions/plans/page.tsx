  ...
  return (
    <div className="relative min-h-screen space-y-8 pb-12">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-3xl font-black text-brand-dark tracking-tight">Subscription Plans</h1>
          <p className="mt-2 text-base text-brand-muted max-w-xl leading-relaxed">
            Configure plan limits, modules, features, and pricing tiers to scale with your business.
          </p>
        </div>
      ...
      <div className="relative z-10 w-full">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8">
          ...
          <div className="relative w-full sm:w-80 group">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-slate-400 group-focus-within:text-brand-primary transition-colors" />
            <input
              type="text"
              placeholder="Search plans..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-11 pr-4 py-3 rounded-full border border-slate-200/60 bg-white/60 backdrop-blur-md text-sm font-medium focus:bg-white focus:border-brand-primary focus:outline-none focus:ring-4 focus:ring-brand-primary/10 transition-all shadow-sm"
            />
          </div>
        </div>
        ...
        <div>
          {filteredPlans.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center bg-white/40 backdrop-blur-xl border border-white/60 rounded-3xl shadow-glass">
              ...
              <h3 className="text-xl font-black text-brand-dark">No plans found</h3>
              <p className="text-base text-brand-muted mt-2 max-w-sm">Try adjusting your search terms or switch to a different tab.</p>
            </div>
          )}
        </div>
      </div>
