...
export async function POST(req: Request) {
  try {
    ...
    const pdfBuffer = await new Promise<Buffer>((resolve, reject) => {
      ...
      const accentColor = '#8cc63f'; // green
      
      const width = doc.page.width;
      
      // 1. Header Design
      // Dark blue background top-left
      doc.polygon(
        [0, 0],
        [width * 0.45, 0],
        [width * 0.35, 120],
      ...
      doc.fillColor(accentColor)
         .fontSize(32)
         .font('Helvetica-Bold')
         .text('INVOICE', 0, 45, { align: 'right', width: width - 40 });
         
      // Invoice Details (Top Right under INVOICE)
      doc.fillColor('#333333')
         .fontSize(10)
         .font('Helvetica-Bold')
         .text('Invoice Number:', width - 200, 90)
         .font('Helvetica')
         .text(`INV-${invoice.id.split('-')[0].toUpperCase()}`, width - 100, 90);
         
      doc.font('Helvetica-Bold')
         .text('Invoice Date:', width - 200, 105)
         .font('Helvetica')
         .text(new Date(invoice.issuedAt).toLocaleDateString(), width - 100, 105);
      ...
      // Invoice To
      doc.text('Invoice To:', 40, startY);
      doc.font('Helvetica-Bold').fillColor('#333333').fontSize(14)
         .text(invoice.businessName || 'Business Client', 40, startY + 15);
      ...
      // Invoice From
      doc.text('Invoice From:', width / 2 + 50, startY);
      doc.font('Helvetica-Bold').fillColor('#333333').fontSize(14)
      ...
      doc.text('PRODUCT DESCRIPTION', 100, tableTop + 10);
      ...
      const items = [
        { desc: `Subscription Plan: ${invoice.plan || 'Standard'}`, details: 'Monthly recurring subscription fee', price: Number(invoice.amount) || 0, qty: 1 }
      ];
      ...
      doc.text('Total', width - 200, summaryY + 60);
      ...
      doc.text('Thank You For Your Business', 0, footerY + 25, { width: width - 40, align: 'right' });
